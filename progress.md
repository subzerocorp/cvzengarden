...
- **Custom domains, first attempt** (NAT-351): both zones are Active on Cloudflare (`bonnie`/`felipe.ns.cloudflare.com`), but the zone scan imported Epik's parking records, and Workers custom domains refuse a hostname with externally managed records (`code 100117`). Adding `routes` without `workers_dev = true` also switched the workers.dev URL off for one deploy (minutes of downtime, restored by redeploy). `wrangler.toml` now pins `workers_dev = true` and `preview_urls = true`; the four `custom_domain` routes go in once the parking records are deleted.
- **Custom domains live** (NAT-351): after the imported parking records were deleted, `wrangler deploy` attached `cvzengarden.com`, `www.cvzengarden.com`, `resumezengarden.com` and `www.resumezengarden.com` as Workers custom domains (decision: both domains serve the same app, no redirect). README's BAR-X3 line names `cvzengarden.com`; workers.dev stays as the fallback.

## 2026-09-16 — Branch previews (NAT-397)

- `[env.preview]` in `wrangler.toml` (`workers_dev`, `RZ_SEED_DEMO = "1"`, `routes = []`); `just preview-db | deploy-preview | destroy-preview <n>` create a Turso database `cvzengarden-pr-<n>` (30-day token, fresh reviewer token) and deploy `cvzengarden-pr-<n>` on workers.dev. CI jobs `preview` (opened / synchronize / reopened, comments the URL via `gh`) and `preview-cleanup` (closed).
- Lesson recorded: a wrangler environment inherits top-level `routes`; the first test deploy moved the custom domains onto the preview Worker for about a minute until production was redeployed. `routes = []` in the environment prevents it. `wrangler secret bulk --env preview --name X` targets `X-preview`, so name-based commands pass `--env=""`.
- Verified from the session with PR number 0: preview served the three official Themes plus the three demo submissions from its own database; production unchanged; `destroy-preview` removed Worker and database.
- **CI concurrency fix**: the `closed` pull-request run shared the concurrency group `ci-refs/heads/main` with the push that the merge produced and cancelled it (run 9 for PR #38, cancelled after one second; production was unaffected only because the same configuration had been deployed by hand). Groups are now keyed by event and PR number or branch.

...
- Product: **ResumeZen**
- Live garden: **https://cvzengarden.com** (BAR-X3). **https://resumezengarden.com** serves the same app, as do both `www` hosts; https://cvzengarden.resumezen.workers.dev is the fallback address. All are the Cloudflare Worker described under Deploy. The old Netlify preview at cvzengarden.netlify.app is superseded.
...
`just worker` runs it locally under wrangler (put the three values in `.dev.vars`). Bun remains the local runtime for `just serve`, the tests and the probes. The Worker answers on `cvzengarden.com`, `resumezengarden.com`, both `www` hosts and `cvzengarden.resumezen.workers.dev`; the custom domains are the `routes` in `wrangler.toml`, and the zones live on Cloudflare (nameservers moved from Epik, 2026-09-16).
